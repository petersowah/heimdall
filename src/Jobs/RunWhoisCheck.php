...
use PeterSowah\Heimdall\Services\Checkers\WhoisCheckerService;
...
class RunWhoisCheck implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(
        ...
        if ($this->domain->notify_domain_expiry && in_array($result['status'], ['warning', 'critical', 'error'])) {
            $alertType = "domain_expiry_{$result['status']}";
